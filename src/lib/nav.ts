 ...
 */
function practiceTopicsHref(exam?: Exam): string {
  const topicId = exam?.focusTopicId ?? exam?.items[0]?.topicId;
  const surprise = exam?.randomScope === "surprise" || (topicId && topicById.get(topicId)?.category === "surprise");
  return surprise ? "/topics?category=surprise" : "/topics";
}

export function examExitLink(mode: Exam["mode"], exam?: Exam): NavLink {
  return mode === "full"
    ? { href: "/exam?mode=full", label: "← 실전 모의고사" }
    : { href: practiceTopicsHref(exam), label: "← 주제별 연습" };
}

/** 연습을 마친 뒤 이어서 하기 좋은 곳. 결과 화면에서 다음 행동으로 보여 준다. */
export function nextPracticeLink(mode: Exam["mode"], exam?: Exam): NavLink {
  return mode === "full"
    ? { href: "/topics", label: "주제별 연습 →" }
    : { href: practiceTopicsHref(exam), label: "다른 주제 연습하기 →" };
}
